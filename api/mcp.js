...
function createServer() {
  ...
  server.resource("devis-widget", "ui://widget/devis.html", async () => ({
    contents: [
      {
  ...
  server.tool(
    ...
    async ({
      ...
      valeur_vehicule,
      annee_vehicule,
      nb_sinistres_responsable,
    }) => {
      const maj =
        nb_sinistres_responsable >= 2
          ? 1.35
      ...
      const tp = Math.round(base * 0.9);
      const tr = Math.round(base * 1.2);
  ...
  server.tool(
    ...
    async ({
      ...
      const options = [];
      if (protection_conducteur) {
        prime += 45;
        options.push("Protection conducteur +45 EUR");
      }
      if (assistance_0km) {
        prime += 35;
        options.push("Assistance 0km +35 EUR");
      }
      if (vehicule_remplacement) {
        prime += 60;
        options.push("Vehicule remplacement +60 EUR");
      }
      ...
      // ✅ LA CORRECTION: toolOutput (racine) + outputTemplate
      return {
        content: [
          {
            type: "text",
